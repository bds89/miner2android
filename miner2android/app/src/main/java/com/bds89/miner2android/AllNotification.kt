...
//                R.drawable.mining)) // большая картинка
    ...
    private fun check_limits() {
        ...
        //try to load PCList
        try {
            val file = FileInputStream("$dir/${const.KEY_SavePC}")
            val inStream = ObjectInputStream(file)
            PCList_load = inStream.readObject() as ArrayList<PC>
            inStream.close()
            file.close()
        ...
        try {
            ...
            val inStream = ObjectInputStream(file)
            all_notification = inStream.readObject() as HashMap<String, MutableMap<String, String>>
            inStream.close()
            file.close()
        } catch (e: Exception) {}
        if (!PCList_load.isNullOrEmpty()) {
            PCList_load.forEach { PC ->
                if (PC.in_IP == "") {
                    if (!ips.containsKey("${PC.ex_IP}:${PC.port}")) ips.put("${PC.ex_IP}:${PC.port}", mutableMapOf())
                } else if (PC.ex_IP == "") {
                    if (!ips.containsKey("${PC.in_IP}:${PC.in_port}")) ips.put("${PC.in_IP}:${PC.in_port}", mutableMapOf())
                } else {
                    if (!ips.containsKey("${PC.ex_IP}:${PC.port}")) ips.put("${PC.ex_IP}:${PC.port}", mutableMapOf(PC.name to "${PC.in_IP}:${PC.in_port}"))
                    else ips["${PC.ex_IP}:${PC.port}"]?.put(PC.name, "${PC.in_IP}:${PC.in_port}")
                }
            }
                ...
                if (value is Int) last_resonce_time.put(name, value)
                    ...
                    if (!(value as JSONObject).has("code")) {
                        value.keys().forEach { gpu ->
                            last_resonce_time.put(name, (System.currentTimeMillis() / 1000).toInt())
                            ...
                            (type).keys().forEach { param ->
                                ...
                                if (const.namesofparams.containsKey(param)) {
                                    textKey += ". ${const.namesofparams[param]?.let { applicationContext.getString(it) }}: "
                                ...
                                if (!textKey.isNullOrEmpty()) {
                                    if (all_notification.containsKey(name)) all_notification[name]?.put(textKey, textValue)
                                    else {
                                        all_notification.put(name, mutableMapOf(textKey to textValue))
            ...
            last_resonce_time.forEach { name, value ->
                ...
                if (tdelta>(31*60) && internetIsConnected()) {
                    if (all_notification.containsKey(name)) all_notification[name]?.put(applicationContext.getString(R.string.no_responce), textTime)
                    else {
                        all_notification.put(name, mutableMapOf(applicationContext.getString(R.string.no_responce) to textTime))
                        silentNotifications = false
                    }
                    temp.add(name)
                    wasNewNotifications = true
